...
var path_number_odds = [75,75,60,40]
var direction_opposites:Dictionary
...
#region methods
func _ready() -> void:
	direction_opposites[directions.north] = directions.south
	direction_opposites[directions.east] = directions.west
	direction_opposites[directions.south] = directions.north
	direction_opposites[directions.west] = directions.east
...
func generate_map(level:int):
	for each_tile in _get_all_tiles(0):
		...
		if possible_new_path_num > 0:
			...
			if roll_for_new_path <= path_number_odds[4 - possible_new_path_num]:
				var new_path = path.new()
				var new_path_direction_from_this_tile = possible_tile_connections_by_path.keys().pick_random()
				new_path.add_connections(each_tile, possible_tile_connections_by_path[new_path_direction_from_this_tile])
				each_tile.set_path(new_path_direction_from_this_tile, new_path)
				possible_tile_connections_by_path[new_path_direction_from_this_tile].set_path(direction_opposites[new_path_direction_from_this_tile], new_path)
		#testing visibility
		each_tile.reveal()
		#
