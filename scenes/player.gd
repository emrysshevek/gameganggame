...
signal move_requested(which_player, old_coord, new_coord)
...
func request_move(new_coordinates:Vector2):
	move_requested.emit(self, coordinates, new_coordinates)
	
func complete_move(new_coordinates):
